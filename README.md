...
- 各端末で「同期 ON / OFF」を独立に切替：ON は **スピーカー**（自分の端末で再生）、OFF は **リモコン**（音は鳴らさず再生・スキップ操作だけ送る）
- 後から同期 ON にした端末は、ルームの先輩 listener から大さっぱな再生位置を引き継いで参加する
...
- Phase 3: per-user 同期トグル（リモコン / スピーカーの分離・初回位置同期） — 実装済
