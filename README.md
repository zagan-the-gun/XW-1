...
- ルーム URL を共有してみんなで曲を追加できる
- 各端末で「聴く ON / OFF」を独立に切替：ON は **スピーカー**（自分の端末で再生）、OFF は **リモコン**（音は鳴らさず再生・スキップ操作だけ送る）
- 後から「聴く」を ON にした端末は、ルームの先輩 listener から大さっぱな再生位置を引き継いで参加する
...
- Phase 1: 基盤（URL追加、順次再生） — 実装済
- Phase 2: リアルタイム共有（Socket.io、キュー同期） — 実装済
- Phase 3: per-user 「聴く」トグル（リモコン / スピーカーの分離・初回位置同期） — 実装済
